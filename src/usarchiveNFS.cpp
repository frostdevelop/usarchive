...
void usarchive::addFile(const std::string &name){
    usa::file filedat;
    std::ifstream infile(stringtools::trimEnds(name),IOSBIN);
    if(!infile.is_open()){infile.close();err=6;return;} //throw std::invalid_argument("USARCHIVE: File not open!");
    //size_t filesize;
    filedat.namesize = stringtools::trimEnds(name).size();
    filedat.name = new unsigned char[filedat.namesize];
    strncpy(reinterpret_cast<char*>(filedat.name),stringtools::trimEnds(name).c_str(),filedat.namesize); //sizeof(filedat.name)
...
void usarchive::unArchive(const std::string &path){
    for(size_t i=0;i<files.size();i++){
        std::ofstream outfile(stringtools::trimEnds(path) + std::string(reinterpret_cast<char *>(files[i].name),files[i].namesize),IOSBIN);
